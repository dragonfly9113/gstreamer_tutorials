...
typedef struct _CustomData
{
    ...
    gdouble rate;               /* Current playing rate (can be negative) */
} CustomData;
...
send_seek_event (CustomData *data)
{
    ...
    /* Obatain the current position, needed for the seek event */
    ...
    if (data->video_sink == NULL) {
        /* If we have not done so, obatain the sink through which we will send the seek events */
...
handle_keyboard (GIOChannel *source, GIOCondition cond, CustomData *data)
{
    ...
    switch (g_ascii_tolower (str[0])) {
        ...
        case 'n':
            if (data->video_sink == NULL) {
                /* If we have not done so, obtain the sink though which we will send the step events */
                g_object_get (data->pipeline, "video-sink", &data->video_sink, NULL);
            }
