...
int main(int argc, char *argv[]) {
  ...
  if (!data.pipeline || !data.source || !data.convert || !data.resample || !data.sink || !data.vsink || !data.vconvert) {
  ...
  gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.convert, data.resample, data.sink, data.vsink, data.vconvert, NULL); 
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
  ...
  if (g_str_has_prefix (new_pad_type, "audio/x-raw")) {
    ...
    if (gst_pad_is_linked (sink_pad)) {
      g_print ("Audio sink pad is already linked. Ignoring.\n");
      goto exit;
    }
    ...
    if (gst_pad_is_linked (vsink_pad)) {
      g_print ("Video sink pad is already linked. Ignoring.\n");
      goto exit;
    }
